...
class PodmanCLI {
    ...
    int createPod(string podName, string[] ports, string[] hostMaps) {
        string args = "";
        foreach(p; ports) args ~= " -p " ~ p;
        foreach(h; hostMaps) args ~= " --add-host " ~ h;
    ...
    int runContainer(string podName, string containerName, string image, string[] envs, string[] volumes, string user, string[] cmdArgs) {
        string args = " --pod " ~ podName ~ " --name " ~ containerName ~ " -d";
        foreach(e; envs) args ~= " -e \"" ~ e ~ "\""; 
        foreach(v; volumes) args ~= " -v " ~ v;
        if (user != "") args ~= " --user " ~ user;
        
        string command = "";
        foreach(c; cmdArgs) command ~= " " ~ c;
    ...
    int ps(string podName) {
        return executeStream("podman ps --filter pod=" ~ podName);
    }
